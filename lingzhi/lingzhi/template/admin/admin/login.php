...
body {background:#15486E url({$skin_path}/login_bg.gif) left top;}
#login {width:788px;height:155px;margin:0px auto;padding:0px auto;background:url({$skin_path}/login_line.gif) 350px 1px no-repeat;font-size:12px;color:white;text-align:left;}
#login p {line-height:22px;font-size:14px; height:28px;}
.input {width:148px;height:22px;line-height:22px;padding:0px 8px;font-size:14px;font-weight:bold;background:#E1DFDF url({$skin_path}/login_btn.gif) repeat-x left top;border:1px solid #002047;}
...
<div id="login">
  ...
  <div style="float:left;width:330px;margin:40px 0px 15px;padding:44px 0px 0px;color:#FFF;background:url({$skin_path}/login_logo.png) no-repeat !important; 
/*For Firefox*/
*background:none;
/*For IE7 & IE6*/
_filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src='{$skin_path}/login_logo.png',sizingMethod='crop');">
...
<?php
    if($loginfalse){
?>
     <p>验&nbsp;&nbsp;&nbsp;&nbsp;证: <input type='text' id="verify"  tabindex="3"  name="verify" class="input" style="width:80px;" />&nbsp;{verify()}</p>
<?php
	}
?>
    ...
    if(get('submit')) {		  
	  if(hasflash()) {	
		  ...
		  echo flash();
            ...
            登陆成功！
            <meta http-equiv="refresh" content="2;url={$admin_url}">
<?php
      }
	  echo '</div>';
	} 
