...
<div id="position">
<a href="javascript:history.back(-1)" title="返回上一页"><img alt="返回上一页" src="{$skin_path}/undo.gif" style="float:right;" /></a>当前位置：编辑投票
</div>

<div class="padding10"><img src="{$skin_path}/wj.gif" style="margin-right:10px;" />欢迎来到投票列表页面。 调用方法： <strong>{</strong>ballot(编号)<strong>}</strong></div>
...
<tbody>
    ...
    <tr class="s_out" onmouseover="m_over(this)" onmouseout="m_out(this)">
      ...
      <td>{cut($d['title'])}</td>
      <td>{cut($d['num'])}</td>
      <td align="center"><a href="<?php echo modify("act/list/table/option/bid/$d[$primary_key]");?>">选项管理</a> <a href="<?php echo modify("act/edit/table/$table/id/$d[$primary_key]");?>">编辑</a> <a onclick="javascript: return confirm('确实要删除吗?');" href="<?php echo modify("/act/delete/table/$table/id/$d[$primary_key]");?>">删除</a></td>
    </tr>
    {/loop}
    </tbody>
