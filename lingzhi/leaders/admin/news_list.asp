...
sql="select id,newstitle,indate,category from news where 1=1 "

If NOT request.form Then
	dim selectKey,searchKey
	selectKey=request("selectKey")
	searchKey=request("searchKey")


	If NOT isNE(searchKey) Then
		sql=sql&"and ( NewsTitle like '%"&searchKey&"%' or Content like '%"&searchKey&"%') "
	End if
	
End if

sql=sql&"order by indate desc"
    ...
    <td width="84%" rowspan="2" valign="top"><table width="100%"  border="0" cellspacing="5" cellpadding="5">
      <tr>
        <td><table width="100%"  border="1" cellpadding="3" cellspacing="0" bordercolor="#CCCCCC" style="border-collapse: collapse">
          <tr align="left">
            <td width="100" align="right" bgcolor="#CCCCCC" >新闻管理</td>
            <td align="right" ><form action="" method="get" name="formSearch" id="formSearch">
              <table width="100%"  border="0" cellspacing="0" cellpadding="0">
                <tr>
                  ...
                  <td align="right"><input name="add_news" type="button" class="button" id="add_news" onClick="location='news_edit.asp'" value="发布新闻"></td>
                </tr>
              </table>
            </form>
            </td>
      ...
      <tr>
		...
		<table width="100%"  border="1" cellpadding="5" cellspacing="0" bordercolor="#CCCCCC" style="border-collapse: collapse">
          <tr align="center" bgcolor="#003366">
            ...
            <td  class="bai">标题</td>
            <td class="bai">所属分类</td>
            <td width="130" class="bai">发布日期</td>
          ...
          <tr style="cursor:pointer" id="<%=rs("id")%>" onmouseover="setPointer(this, 'over');" onmouseout="setPointer(this, 'out');" onmousedown="setPointer(this, 'click')">
            ...
            <td title="<%=rs("newstitle")%>"><%=substr(rs("newstitle"),30)%></td>
            <td align="center">
			<%
			if NOT isNE(rs("category")) then response.write newsCategory(rs("category"))
			%></td>
            <td align="center"><%=rs("indate")%></td>
			
            <td align="center">[<a href="news_edit.asp?action=edit&id=<%=rs("id")%>">编辑</a>][<a href="operation.asp?action=delnews&id=<%=rs("id")%>">删除</a>]</td>
          </tr>
		  <%
		  rs.movenext
		  wend
		  %>
          <tr align="center">
            <td colspan="5"> <%=classPager.getBar%></td>
            </tr>
        </table></td>
      </tr>
      <tr>
