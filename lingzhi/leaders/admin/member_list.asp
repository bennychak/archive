...
sql="select ID,Country,Company,RegDate,LastLogin,LoginId,WhatsNew from members where 1=1 "
If NOT request.form Then
	...
	If NOT isNE(searchKey) Then
		...
		sql=sql&"or LoginId like '%"&searchKey&"%' "
		sql=sql&"or FirstName like '%"&searchKey&"%' "
		sql=sql&"or LastName like '%"&searchKey&"%' "
		sql=sql&"or JobTitle like '%"&searchKey&"%' "
		sql=sql&"or Company like '%"&searchKey&"%' "
		sql=sql&"or City like '%"&searchKey&"%' "
		sql=sql&"or Province like '%"&searchKey&"%' "
		sql=sql&"or Address like '%"&searchKey&"%' "
		sql=sql&"or Country like '%"&searchKey&"%' "
		sql=sql&"or Industry like '%"&searchKey&"%' "
		sql=sql&"or Description like '%"&searchKey&"%' "
		sql=sql&"or Email like '%"&searchKey&"%') "
	End if
    ...
    <td width="84%" rowspan="2" valign="top"><table width="100%"  border="0" cellspacing="5" cellpadding="5">
      <tr>
        <td><table width="100%"  border="1" cellpadding="3" cellspacing="0" bordercolor="#CCCCCC" style="border-collapse: collapse">
          <tr align="left">
            <td width="100" align="right" bgcolor="#CCCCCC" >会员管理</td>
      ...
      <tr>
		...
		<table width="100%"  border="1" cellpadding="5" cellspacing="0" bordercolor="#CCCCCC" style="border-collapse: collapse">
          <tr align="center" bgcolor="#003366">
            ...
            <td  class="bai">登录ID</td>
            <td  class="bai">公司</td>
			<td width="120" class="bai">国家</td>
			<td bgcolor="#003366" class="bai">注册日期</td>
            <td width="120" class="bai">最后登陆</td>
            <td width="120" class="bai">WHAT'S NEW </td>
            <td width="120" class="bai">操作</td>
          </tr>
		  <%
          ...
          <tr style="cursor:pointer" id="<%=rs("ID")%>" onmouseover="setPointer(this, 'over');" onmouseout="setPointer(this, 'out');" onmousedown="setPointer(this, 'click')">
            ...
            <td align="center">
			<%
			If rs("WhatsNew")="1" Then
				response.Write "<a href=operation.asp?action=setWhatsnewLevel&id="&rs("ID")&"&whatsnew=0&urlPara="&urlPara&">√</a>"
			Else
				response.Write "<a href=operation.asp?action=setWhatsnewLevel&id="&rs("ID")&"&whatsnew=1&urlPara="&urlPara&">×</a>"
			End if
			%>			
			</td>
            <td align="center">[<a href="mail_new.asp?userID=<%=rs("ID")%>&returnPage=<%=thisPage%>">邮件</a>][<a href="member_edit.asp?action=edit&id=<%=rs("id")%>">编辑</a>][<a href="operation.asp?action=delMember&id=<%=rs("id")%>">删除</a>]</td>
          </tr>
		  <%
		  rs.movenext
		  wend
