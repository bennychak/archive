//$Id: checkForm.js,v 1.5 2003/07/23 02:14:00 sunbin Exp $
 ...
 */
	function checkForm(fm)
	{
		for(var i=0;i<fm.length;i++)
		{
				...
				else
				{
					...
					if(!isNaN(lengthLimit))
					{
						if(value.length>lengthLimit)
						{
							...
							alert(info+"\n你所输入的长度("+value.length+")超过限制"+lengthLimit);
							fm[i].focus();
							return false;
						}
					}
			...
			{
				//日期时间
				var found=value.match(/(\d{1,5})-(\d{1,2})-(\d{1,2}) (\d{1,2}):(\d{1,2}):(\d{1,2})/);
				if(found==null||found[0]!=value||found[2]>12||found[3]>31||found[4]>23||found[5]>59||found[6]>59)
				{
