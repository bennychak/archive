 ...
 <script type="text/javascript"> 
        $(document).ready(function(){
            ...
            var settings = {
                ...
                upload_url: "<?php echo url('tool/uploadimage',false);?>",
...
<div class="padding10">
    <img src="<?php echo $skin_path;?>/wj.gif" style="margin-right:10px;" />欢迎来到添加内容页面。您可以添加网站内容，展示网站信息。您可以填写不同的内容网页标题，配合关键词以便更好的SEO页面。
</div>
...
<form method="post" name="form1" action="<?php if(front::$act=='edit') $id="/id/".$data[$primary_key]; else $id='';
    ...
    <script type="text/javascript" src="<?php echo $base_url;?>/common/swfupload/handlers.js"></script>
...
<div id="nava">
    <div class="hid_box">
        ...
        <div id="ChildMenu1" class="collapseda" style="padding:20px 50px;">
            网页标题：<br /><?php echo form::getform('mtitle',$form,$field,$data);?><br /><br />
            关&nbsp;键&nbsp;&nbsp;词 (keywords)：<br /><?php echo form::getform('keyword',$form,$field,$data);?><br /><br />
            页面描述 (description)：<br /><?php echo form::getform('description',$form,$field,$data);?><br /><br />
            内容简介字数限制：<br /><?php echo form::getform('introduce_len',$form,$field,$data);?><br />
    ...
    <div class="hid_box">
       ...
       <div  id="ChildMenu3" class="collapseda" style="padding:20px 50px;">
            <?php echo form::getform('checked',$form,$field,$data);?> <div class="clear"></div>
        </div>
    </div>
...
<div class="hid_box">
    ...
    <div class="hid_box">
        ...
        <div  id="ChildMenu8" class="collapseda" style="padding:20px 50px;">
            ...
            <input type="hidden" name="attachment_id"  id="attachment_id" value="<?php echo archive_attachment(@$data['aid'],'id');?>" size="50"/>
            描述：<input type="text" name="attachment_intro"  id="attachment_intro" value="<?php echo archive_attachment(@$data['aid'],'intro');?>" size="50"/><br><br>
            保存路径：<span id="attachment_path"><?php echo archive_attachment(@$data['aid'],'path');?></span> <input type="button" name="delbutton" value="删除" onclick="attachment_delect(get('attachment_id').value)"><br><br>
            <?php if(front::$act=='edit' && $data['attachment_id']) { ?>
            更改：<?php } ?>

            上传：<input type="file" name="fileupload" id="fileupload" style="width:400px" />
            &nbsp;&nbsp;<input type="button"  name="filebuttonUpload"  id="filebuttonUpload" onclick="return ajaxFileUpload('fileupload','<?php echo url("tool/uploadfile",false);?>','#uploading');" value="上传" />
    ...
    <div class="hid_box">
        ...
        <ul  id="ChildMenu9" class="collapseda" style="padding:20px 50px;">
            <?php for($i=1;$i<=8;$i++) { ?>
            <li><?php echo $i;?> &nbsp; 标题 <?php echo form::input("vote[$i]",vote::title(@$data['aid'],$i));?>
                &nbsp; 图片url <?php echo form::input("vote_image[$i]",vote::img(@$data['aid'],$i));?>
                    <?php } ?>
        </ul>
    </div>
    ...
    <div class="hid_box">
        ...
        <div  id="ChildMenu11" class="collapseda" style="padding:20px 50px;">
            <table width="90%">
                <tr><td width="20%">评分：<?php echo form::getform('grade',$form,$field,$data);?></td>
                </tr>
            </table>
        </div>
    </div>
    ...
    <div class="hid_box">
        ...
        <div  id="ChildMenu12" class="collapseda" style="padding:20px 50px;">
            <DIV id=uploadarea>
              ...
              <BR>上传：<INPUT style="WIDTH: 400px" id="pics0_upload" onchange="image_preview('pics0',this.value,1)" type="file" name="pics0_upload">&nbsp;&nbsp;<INPUT id="pics0upload" onclick="return ajaxFileUpload3('pics0_upload','<?php echo url('tool/upload2',false);?>','#pics0_loading',1);" value="上传" type="button" name="pics0upload">
              </DIV>
            </DIV>
        </div>
    </div>
    ...
    <div class="hid_box">
     ...
     <div class="hid_box">
        ...
        <?php echo form::getform('attr2',$form,$field,$data);?> 建议格式：￥1000/件
        </div>
    </div>
