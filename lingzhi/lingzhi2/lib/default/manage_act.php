 ...
 * @version:    v1.7.091010
 * ---------------------------------------------
 * $Id: manage_act.php Sat Dec 06 09:43:43 CST 2008
 */
    ...
    function edit_action() {
        ...
        if(front::post('submit') && $this->manage->vaild()) {
            ...
            if($update<1) {
                ...
                front::flash('记录修改成功！');
                $from=session::get('from'); session::del('from');
                header("Location: ".$from, TRUE, 302);
				exit;
            }
        }
...
/*	function batch_action() {
        ...
        if(front::post('batch') && front::post('select')) {
            ...
            if(front::post('batch')=='delete') {
                ...
                if($delete>0) front::flash("成功删除记录！");
                else front::flash("审没有记录被删除！");
            }
            else{
            }
        }
