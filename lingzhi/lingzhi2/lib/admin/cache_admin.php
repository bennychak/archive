 ...
 * @version:    v1.7.091010
 * ---------------------------------------------
 * $Id: cache_admin.php Sat Dec 06 09:40:09 CST 2008
 */
...
class cache_admin extends admin {
    ...
    function make_list_action() {
        ...
        foreach ($categories as $key=>$catid) {
            ...
            $archive_num[]=$archive_all->rec_count('catid in('.implode(',', $new_categories).') and checked=1');
            $archives[]=$archive_all->getrows('catid in('.implode(',', $new_categories).') and checked=1', '0,'.config::get('list_pagesize'), '`order` asc,aid desc');
        }
        $i=0;
        foreach ($categories as $catid) {
            ...
            for (front::$get['page']=1;; front::$get['page']++) {
                ...
                $archive=new archive();
                if (@$category->category[$catid]['includecatarchives']) $case->view->archives=$archive->getrows('catid in('.implode(',', $_categories).') and checked=1', $limit, '`listorder` asc,aid desc');
                else $case->view->archives=$archive->getrows("catid=$catid and checked=1", $limit, '`listorder` desc,aid desc');
                ...
                if (!isset($page_count)) {
                    $case->view->record_count=$archive_num[$i];
                    $case->view->page_count=ceil($case->view->record_count / $pagesize);
                    front::$record_count=$case->view->record_count;
                ...
                /* if(!$category->parent[$catid] && front::get('page')>1)  {
                  $page_count=null;
                  break;
                  } */
    ...
    function make_show_action() {
        ...
        $front=new front();
        //	front::$get=array_merge(front::$ca,$var);
        front::$admin=false;
        front::$html=true;
        front::$rewrite=false;
        ...
        foreach ($archives as $arc) {
            ...
            foreach ($linkwords as $linkword) {
                ...
                if (isset($link)) $content=str_replace("'", "\"", $content);
