...
<div align="center">
  ...
  <table width="720" border="0" cellpadding="3" cellspacing="1" bgcolor="#6298E1">
    <tr>
      <td height="24" colspan="4"><font color="#FFFFFF"><strong>访问者来访网站分析&nbsp;</strong>(&nbsp;总量：<%=TotalNum%>&nbsp;)&nbsp;&nbsp;&nbsp;&nbsp;<a href="FMozilla.asp"> <font color="#FFFFFF">[&nbsp;链接页面分析&nbsp;]</font></a></font></td>
    </tr>
    <tr bgcolor="#D7E4F7">
      <td width="44%">链接页面</td>
      <td width="8%">访问人数</td>
    ...
    <tr bgcolor="#EBF2F9" onMouseOver = 'this.style.backgroundColor = "#FFFFFF"' onMouseOut = 'this.style.backgroundColor = ""' style="cursor:hand">
      <td><% if Instr(  Assay(0,i) ,"dxxo") then
			 ExFor=ExFor-1
			 else%>
        <% If Assay(0,i)="直接输入或书签导入" Then%>
        <%=Left(Assay(0,i),46)%>
        <%Else%>
        <a href="<%=Assay(0,i)%>"><%=Left(Assay(0,i),46)%></a>
      <% end if %></td>
      <td><%=Assay(1,i)%></td>
      <td><%=Percent(i)%></td>
