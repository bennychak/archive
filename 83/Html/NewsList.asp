...
<div class="sub">
	<div class="content">
        ...
        <div class="right">
        	...
        	<div class="con"><%=WebContent("NwebCn_NewsSort",request.QueryString("SortID"),"")%></div>
        </div>
        <div class="clear"></div>
    </div>
</div>
  ...
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while not rs.eof '填充数据到表格
	  if rs("PictureFlag") then 
        response.write "·<font color=""#0000ff"">[图]</font><a href=""NewsView.asp?ID="&rs("ID")&""">"&rs("NewsName")&"</a>&nbsp;<span class=""Black-bbb"">"&FormatDate(rs("Addtime"),14)&"</span><br />"
	  else
