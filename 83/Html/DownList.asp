<!--#include file="Head.asp"-->
  <div id="Bodyer_page">
	...
	<div id="Bodyer_right_page">
      ...
      <%=WebContent("NwebCn_DownSort",request.QueryString("SortID"),"")%>
	</div>
  </div>
<%
...
function WebLocation()
  ...
  elseif conn.execute("select * from NwebCn_DownSort Where ViewFlag and  ID="&request.QueryString("SortID")).eof then
    WebLocation=WebLocation&"<img src=""../Images/Arrow_03.gif"" />信息ID读取错误"
  else
    dim rs,sql
    set rs = server.createobject("adodb.recordset")
	sql="select * from NwebCn_DownSort where ViewFlag and ID="&request.QueryString("SortID")
    rs.open sql,conn,1,1
	WebLocation=WebLocation&SortPathTXT("NwebCn_DownSort",rs("ID"))
    rs.close
    set rs=nothing
  end if
end function
