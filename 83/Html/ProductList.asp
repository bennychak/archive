<!--#include file="Head.asp"-->
  <div id="Bodyer_page">
	...
	<div id="Bodyer_left_page">
	  ...
	  <div class="Bodyer_left_page_menu"><%=WebMenu(0,0,3)%></div>
	</div>
	<div id="Bodyer_right_page">
      <div class="Bodyer_right_page_location"><%=WebLocation()%></div>
      <%=WebContent("NwebCn_ProductSort",request.QueryString("SortID"),"")%>
	</div>
  </div>
<%
...
function WebLocation()
  ...
  elseif conn.execute("select * from NwebCn_ProductSort Where ViewFlag and  ID="&request.QueryString("SortID")).eof then
    WebLocation=WebLocation&"<img src=""../Images/Arrow_03.gif"" />信息ID读取错误"
  else
    dim rs,sql
    set rs = server.createobject("adodb.recordset")
	sql="select * from NwebCn_ProductSort where ViewFlag and ID="&request.QueryString("SortID")
    rs.open sql,conn,1,1
	WebLocation=WebLocation&SortPathTXT("NwebCn_ProductSort",rs("ID"))
    rs.close
    set rs=nothing
  end if
end function
...
function WebContent(DataFrom,ID,SortPath)
  ...
  dim datawhere'数据条件
	  datawhere="where ViewFlag and (Instr(SortPath,'"&SortPath&"')>0 or Instr(SortPath2,'"&SortPath&"')>0) "&HideSort& " "
  dim sqlid'本页需要用到的id
  ...
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    response.write "<div class=""Bodyer_right_page_content""><table cellpadding=""0""  cellspacing=""0"">"
	for tr=1 to 3
	    response.write "<tr>"
        for td=1 to 6 '填充数据到表格
		  ...
		  response.write "<td width=""18""></td><td>"
          response.write "<table cellpadding=""2"" cellspacing=""0"" >" &_
				    	 "<tr><td style=""border: 1px solid #ccc; width:124px; height:124px; text-align:center;"">" &_
						 "<a href=""ProductView.asp?ID="&rs("id")&""">" &_
				    	 "<img src="""&SmallPicPath&""" onload=""javascript:DrawImage(this,124,124);"" alt="""&rs("ProductName")&"""  border=""0"" width=""124"" height=""124""/>" &_
						 "</a>" &_
					     "</td></tr>" &_
						 "<tr><td style=""height:28px; text-align:center;"">" &_
