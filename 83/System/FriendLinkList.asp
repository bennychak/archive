...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#6298E1">
  <form action="DelContent.asp?Result=FriendLink" method="post" name="formDel" >
    <tr>
      ...
      <td width="120" nowrap bgcolor="#8DB5E9"><font color="#FFFFFF"><strong>友情链接前台显示</strong></font></td>
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
	  ...
	  if rs("LinkType") then
        Response.Write "<td title="&rs("LinkName")&" nowrap><img align='absmiddle' src='"&rs("LinkFace")&"'></td>" & vbCrLf
      else
        Response.Write "<td title="&rs("LinkName")&" nowrap>"&rs("LinkFace")&"</td>" & vbCrLf
      end if
	  if StrLen(rs("LinkUrl"))>53 then
        Response.Write "<td title="&rs("LinkUrl")&" nowrap>"&StrLeft(rs("LinkUrl"),50)&"</td>" & vbCrLf
      else
        Response.Write "<td title="&rs("LinkUrl")&" nowrap>"&rs("LinkUrl")&"</td>" & vbCrLf
      end if
      Response.Write "<td nowrap>"&rs("AddTime")&"</td>" & vbCrLf
      Response.Write "<td width='48' nowrap><a href='FriendLinkEdit.asp?Result=Modify&ID="&rs("ID")&"' onClick='changeAdminFlag(""修改友情链接"")'><font color='#330099'>修改</font></a></td>" & vbCrLf
    ...
    response.write "<tr><td height='50' align='center' colspan='9' nowrap  bgcolor='#EBF2F9'>暂无友情链接</td></tr>"
  end if
'-----------------------------------------------------------
'-----------------------------------------------------------
  Response.Write "<tr>" & vbCrLf
