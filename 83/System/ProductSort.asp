...
Select Case Action
  ...
  Case "Del"
    ...
    conn.execute("update NwebCn_Products set SortID2=null,SortPath2='' where Instr(SortPath2,'"&SortPath&"')>0")
...
Function Folder(id)
  ...
  Set rs=server.CreateObject("adodb.recordset")
  sql="Select * From NwebCn_ProductSort where ParentID="&id&" order by Sequence"
  rs.open sql,conn,1,1
  ...
  while not rs.eof
    ...
    response.write("<font color='#367BDA'>&nbsp;|&nbsp;</font><a href='noerror.asp'>移</a>")
    response.write("→<a href='noerror.asp'>至</a>")
    ...
    response.write("<font color='#367BDA'>&nbsp;|&nbsp;</font><a href='ProductList.asp?SortID="&rs("ID")&"&SortPath="&rs("SortPath")&"' onClick='changeAdminFlag(""产品列表"")'>列表</a>")
...
Function addFolderForm(ParentID)
  Dim ParentPath,Sequence,SortTextPath,Picture,Summary,rs,sql
  if ParentID=0 then
    ParentPath="0,"
	SortTextPath=""
	Picture="../Images/NoSortPicture.jpg"
	Summary=""
	Sequence=0
  else 
    Set rs=server.CreateObject("adodb.recordset")
  ...
  <tr>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td width="350" nowrap>父类数字路径：<input readonly name="ParentPath" type="text" class="textfield" id="ParentPath" size="45" value="<%=ParentPath%>"></td>
        <td nowrap></td>
	  </tr>
      <tr>
        <td width="190" rowspan="2" nowrap valign="top">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src="<%=Picture%>" name="SortPicture" align="absmiddle" id="SortPicture" style="cursor:pointer;" title="点击更新刚修改的图片..." onClick="UpSortPicture()"/></td>
        <td colspan="4" nowrap >图片：<input name="Picture" type="text" class="textfield" id="Picture" style="WIDTH:518;" maxlength="100" value="../Images/NoSortPicture.jpg">&nbsp;<a href="javaScript:OpenScript('UpFileForm.asp?Result=Picture',460,180)"><img src="Images/Upload.gif" width="30" height="16" border="0" align="absmiddle"></a></td>
	  </tr>
      <tr>
        <td colspan="4" nowrap >概要：<textarea name="Summary" id="Summary" rows="4" class="textfield" style="WIDTH: 555;"><%=Summary%></textarea></td>
	  </tr>
      <tr>
...
Function saveFolder
  ...
  rs("Summary")=StrReplace(request.Form("Summary"))
  rs.update 
...
Function editFolderForm(ID)
  Dim SortName,ViewFlag,ParentID,SortPath,Picture,Summary,Sequence,rs,sql
  Set rs=server.CreateObject("adodb.recordset")
  sql="Select * From NwebCn_ProductSort where ID="&ID
  rs.open sql,conn,1,1
  ...
  SortPath=rs("SortPath")
  Picture=rs("Picture")
  Summary=ReStrReplace(rs("Summary"))
  ...
  <tr>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td width="120" nowrap>父类ID：<input readonly name="ParentID" type="text" class="textfield" id="ParentID" size="6" value="<%=ParentID %>"></td>
        <td width="350" nowrap>父类数字路径：<input readonly name="SortPath" type="text" class="textfield" id="SortPath" size="45" value="<%=SortPath%>"></td>
        <td nowrap></td>
	  </tr>
      <tr>
        <td width="190" rowspan="2" nowrap valign="top">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src="<%=Picture%>" name="SortPicture" align="absmiddle" id="SortPicture" style="cursor:pointer;" title="点击更新刚修改的图片..." onClick="UpSortPicture()"/></td>
