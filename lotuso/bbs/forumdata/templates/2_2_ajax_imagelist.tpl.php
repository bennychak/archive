...
<p class="imginfo">
<span class="right"><a href="javascript:;" onclick="delImgAttach(<?=$image['aid']?>,<? if(!$attach['pid']) { ?>1<? } else { ?>0<? } ?>)" class="deloption">É¾³ý</a></span>
<? if($image['description']) { ?>
<input type="text" name="attachnew[<?=$image['aid']?>][description]" class="txt" value="<?=$image['description']?>" id="image_desc_<?=$image['aid']?>" />
<? } else { ?>
<input type="text" class="txt lighttxt" value="ÃèÊö" onclick="this.style.display='none';$('image_desc_<?=$image['aid']?>').style.display='';$('image_desc_<?=$image['aid']?>').focus();" />
