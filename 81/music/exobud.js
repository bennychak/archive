...
if(blnRndPlay){ document.exobudform.pmode.value="随机"; document.exobudform.pmode.style.background=btnOnColor;}
else { document.exobudform.pmode.value="顺序"; document.exobudform.pmode.style.background=btnOffColor;}
showTLab();
document.exobudform.disp1.value = "Musique de Bienfantaisie";
if(blnStatusBar){ window.status=('Musique de Bienfantaisie');}
...
Exobud.close();
document.exobudform.disp1.value = "Musique de Bienfantaisie [就绪]";
if(blnStatusBar){ window.status=('Musique de Bienfantaisie [就绪]');return true;}
}
...
clearTimeout(tidMsg);
if(wmps==2){wmpc.play();}
if(wmps==3){wmpc.pause(); document.exobudform.disp2.value="暂停"; tidMsg=setTimeout('rtnTLab()',1500);}
return;
}
...
if(blnRept){
blnRept=false; document.exobudform.rept.style.background=btnOffColor; document.exobudform.disp2.value="不重复播放";
} else {
blnRept=true; document.exobudform.rept.style.background=btnOnColor; document.exobudform.disp2.value="重复播放";
}
...
if(f){
document.exobudform.disp2.value = "缓冲处理中";
...
if(intErrCnt<=3){
document.exobudform.disp2.value = "尝试联机 (" + intErrCnt + ")";
var msg = "(尝试第 " + intErrCnt + " 次联机) " + cActTit;
document.exobudform.disp1.value = "<无法播放> " + cActTit;
if(blnStatusBar){ window.status=(msg);}
...
if(wmps==3){clearInterval(tidTLab);}
if(blnElaps){
blnElaps=false; document.exobudform.disp2.value="倒数方式";
} else {
...
if(ps.mute){
ps.mute=false;
document.exobudform.disp2.value="音量恢复";
...
if(!ps.mute){
ps.mute=true;
document.exobudform.disp2.value="开启静音模式";
document.exobudform.vmute.style.background=btnOnColor;
} else {
ps.mute=false;
document.exobudform.disp2.value="关闭静音模式";
document.exobudform.vmute.style.background=btnOffColor;
}
tidMsg=setTimeout('rtnTLab()',1000);
}
...
if(intExobudStat==1){ document.exobudform.disp1.value = "无法播放 － 播放清单上没有设定任何曲目。";}
if(intExobudStat==2){ document.exobudform.disp1.value = "无法播放 － 您没有选取播放清单上任何一首曲目。";}
if(blnStatusBar){
...
window.open("http://Bienfantaisie.veryplace.com","vizExobud");
}
//-->
