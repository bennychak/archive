...
Dim art__MMColParam
art__MMColParam = "1"
If (Request.QueryString("belong") <> "") Then 
  art__MMColParam = Request.QueryString("belong")
End If
%>
<%
Dim art
...
art.Source = "SELECT * FROM art WHERE article_belong = '" + Replace(art__MMColParam, "'", "''") + "' ORDER BY article_time DESC"
art.CursorType = 0
...
%>
          <li><A target="_blank" HREF="article.asp?<%= Server.HTMLEncode(MM_keepNone) & MM_joinChar(MM_keepNone) & "id=" & art.Fields.Item("id").Value %>"><%=(art.Fields.Item("article_time").Value)%>&nbsp;&nbsp;<%=(art.Fields.Item("article_title").Value)%>&nbsp;&nbsp;<%=(art.Fields.Item("article_num").Value)%></A></li>
            <% 
