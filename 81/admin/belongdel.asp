...
belong_total = belong.RecordCount

' set the number of rows displayed on this page
If (belong_numRows < 0) Then
  belong_numRows = belong_total
Elseif (belong_numRows = 0) Then
  belong_numRows = 1
End If

' set the first and last displayed record
belong_first = 1
belong_last  = belong_first + belong_numRows - 1

' if we have the correct record count, check the other stats
If (belong_total <> -1) Then
  If (belong_first > belong_total) Then
    belong_first = belong_total
  End If
  If (belong_last > belong_total) Then
    belong_last = belong_total
  End If
  If (belong_numRows > belong_total) Then
    belong_numRows = belong_total
  End If
End If
%>
...
Set MM_rs    = belong
MM_rsCount   = belong_total
MM_size      = belong_numRows
MM_uniqueCol = "id"
MM_paramName = "id"
MM_offset = 0
MM_atTotal = false
...
' set the first and last displayed record
belong_first = MM_offset + 1
belong_last  = MM_offset + MM_size

If (MM_rsCount <> -1) Then
  If (belong_first > MM_rsCount) Then
    belong_first = MM_rsCount
  End If
  If (belong_last > MM_rsCount) Then
    belong_last = MM_rsCount
  End If
End If
...
<form id="form1" name="form1" method="POST" action="<%=MM_editAction%>">
  <table width="300" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr>
      <td><%=(belong.Fields.Item("belong").Value)%></td>
      <td><label>
  ...
  <input type="hidden" name="MM_recordId" value="<%= belong.Fields.Item("id").Value %>">
</form>
</body>
</html>
<%
