	...
	if(rate_fadein_opacity < 100) {
		...
		if(is_opera)  {
			...
			if(ratings_show_fading) {
				document.getElementById('post-ratings-' + post_id).style.filter="progid:DXImageTransform.Microsoft.Alpha(opacity="+rate_fadein_opacity+")";
			} else {
				rate_fadein_opacity = 100;
			}
		} else	 if(is_moz) {
...
function current_rating(id, rating, rating_text) {
	if(!is_being_rated) {
		...
		if(document.getElementById('ratings_' + post_id + '_text')&&1==2) {
...
function rate_process() {
	if(rate_fadeout_opacity > 0) {
		...
		if(is_opera) {
			...
			if(ratings_show_fading) {
				{var t;
				if(t=document.getElementById('post-ratings-' + post_id))
					
					t.style.filter="progid:DXImageTransform.Microsoft.Alpha(opacity="+rate_fadeout_opacity+")";
				}
			} else {
				rate_fadein_opacity = 0;
			}
